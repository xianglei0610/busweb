...
class Flow(BaseFlow):
    ...
    def request_lock(self, order, rebot, sta_mode=1):
        headers = {
            ...
            "Referer": "http://113.204.53.134/User/CommitGoods.aspx",
        }
        cookies = json.loads(rebot.cookies)
        if sta_mode == 1:
            base_url = "http://113.204.53.134/CommitGoods.aspx"
            ...
            base_url = "http://113.204.53.134/OrderConfirm.aspx"
            r = rebot.http_get(base_url, headers=headers, cookies=cookies)
            soup = BeautifulSoup(r.content, "lxml")
            headers.update({"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"})
            params ={
    ...
    def request_get_shoptcart(self, rebot):
        ...
        获取购物车条目
        """
        base_url = "http://113.204.53.134/UserData/ShopCart.aspx"
        headers = {
            "User-Agent": rebot.user_agent,
    ...
    def request_station_status(self, line, rebot):
        """
            车站状态
        """
        base_url = "http://113.204.53.134/UserData/UserCmd.aspx"
        params = {
    ...
    def get_pay_page(self, order, valid_code="", session=None, pay_channel="alipay" ,**kwargs):
        ...
        if order.status == STATUS_WAITING_ISSUE:
            ...
            order.reload()
            base_url = "http://113.204.53.134/GoodsDetail.aspx"
            headers = {
                "User-Agent": rebot.user_agent,
                "Referer": "http://113.204.53.134/TicketMain.aspx",
                "Origin": "http://113.204.53.134",
            ...
            soup = BeautifulSoup(r.content, "lxml")
            headers.update({"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"})
            headers.update({"Referer": "http://113.204.53.134/GoodsDetail.aspx"})
            params ={
                "__VIEWSTATE": soup.select("#__VIEWSTATE")[0].get("value"),
                "__EVENTVALIDATION": soup.select("#__EVENTVALIDATION")[0].get("value"),
        ...
        #     valid_url = "http://113.204.53.134/ValidateCode.aspx?_=%s" % random.randint(1, 10000)
