...
class Flow(BaseFlow):
    ...
    def do_lock_ticket_by_app(self, order):
        ...
        elif u"班次不在预售期" in errmsg or u"没有余票" in errmsg or u"该班次在客运站" in errmsg or u"锁票失败了 ╮囧╭ 请您再试一次" in errmsg or u"客运站网络异常" in errmsg or u"该班次票价或时间在客运站出现异常" in errmsg:
