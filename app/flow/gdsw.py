...
class Flow(BaseFlow):
    ...
    def do_lock_ticket_by_app(self, order):
        ...
        elif u"班次不在预售期" in errmsg or u"没有余票" in errmsg or u"该班次在客运站" in errmsg or u"锁票失败了 ╮囧╭ 请您再试一次" in errmsg or u"客运站网络异常" in errmsg:
    ...
    def do_refresh_issue_by_web(self, order):
        ...
        try:
            ...
            may_code = order.contact_info["id_number"][-6:]
            ...
            if may_code != pick_code: # 用于监测
                order_log.error("[广东省网] %s 取票密码不一定是身份证后六位, 请检查")
