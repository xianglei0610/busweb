...
def log_request():
    ...
    access_log.info("[request] %s %s %s %s", uname, request.environ.get('HTTP_X_REAL_IP', request.remote_addr), request.url, request.values)
...
def src_code_img(order_no):
    ...
    if order.crawl_source == "bus100":
        ...
        return r.content
    elif order.crawl_source in ["bjky", "cqky", 'scqcp', "changtu", 'xintuyun']:
